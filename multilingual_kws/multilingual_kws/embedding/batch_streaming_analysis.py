...
def calculate_streaming_accuracy(
    ...
    results = []
    # print ("flag list size:", len(flag_list))

    for FLAGS in flag_list:
        res_thresh = {}
        print("detection thresholds:", FLAGS.detection_thresholds)
        for threshold in FLAGS.detection_thresholds:
            ...
            print("calculating stats using inferences output...")
            ...
            for ix, audio_data_offset in enumerate(
                ...
                #view outcomes of this latest result
                rec_comms = vars(recognize_commands)
                prev_res = rec_comms['_previous_results']
                prev_top_label = rec_comms['_previous_top_label']
                prev_top_time = rec_comms['_previous_top_time']

                # print("   ")
                # print("prev_results: ", prev_res )
                # print("prev_top_label: ", prev_top_label)
                # print("prev_top_time: ",prev_top_time)
                # print("   ")
...
def eval_stream_test(st: StreamTarget, live_model=None):
    ...
    if not inferences_exist and st.destination_result_inferences is not None:
        ...
        #round inferences and write into txt file
        inferences = np.around(inferences, decimals=3)
        # print("inferences: ", inferences)
        # print("type: ", type(inferences))
        np.savetxt(st.destination_result_inferences, inferences, fmt="%.2f")
